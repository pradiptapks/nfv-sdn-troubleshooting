...
# Modified:  Pradipta Sahoo  psahoo@redhat.com -- Modified: 2019
# 
# The script has inherited from following Git repo: https://github.com/MellanoxSupport/sysinfo-snapshot/blob/master/sysinfo-snapshot.py
# Enhancement has done for OVS Hardware TC Offload
# For performance Analysis, run the script with below parameters:
# Syntax: ./mlx-sysinfo-snapshot.py --perf --fsdump --mtusb --ibdiagnet --openstack --asap --asap_tc --pcie --check_fw --verbose
#
__author__ = 'nizars'
...
commands_collection = ["ip -s -s link show", "ip -s -s addr show", "rpm -qa --last", "ovs-vsctl --version", "ovs-vsctl show", "ovs-dpctl show","ovs-dpctl show -s", "ovs-appctl coverage/show", "ovs-appctl upcall/show", "ovs-dpctl dump-flows -m type=offloaded", "ovs-dpctl dump-flows -m type=ovs", "pidstat -t -p `pidof ovs-vswitchd`", "brctl --version", "brctl show", "mlxmcg -d", "arp -an", "free", "blkid -c /dev/null | sort", "date", "time", \
                        ...
                        "service cpuspeed status", "service iptables status", "service irqbalance status", "show_irq_affinity_all", "sysctl -a", "tgtadm --mode target --op show", "tgtadm --version", "tuned-adm active", "ulimit -a", "uname -a", "uptime", "cpupower monitor", "cpupower frequency-info", "cpupower idle-info" \
...
def asap_tc_handler():
	...
	cmd1 =  "tc -s filter show dev " + interface + " egress "
        with open(path + file_name + "/asap_tc/ovs_tc_filter_" + interface, "a+") as outF:
            ...
            outF.write(cmd1)
            st, res = get_status_output(cmd1 + " >> " + path + file_name + "/asap_tc/ovs_tc_filter_" + interface, '300s')
            if st != 0:
                outF.write("Could not run: " + cmd1)
...
def mst_func_handler():
    ...
    if is_MST_installed:
        ...
        for card in mlnx_pci_devices:
            ...
            output = card
            filtered_file_name = output.replace(":", "").replace(".", "")
            st, res = get_status_output("mstconfig -a -d " + card + " -e  q > " + path + file_name + "/firmware/mstconfig_" + filtered_file_name, '40s')
